...
def train(model, optimizer, scheduler, global_step,
    ...
    while global_step < opt.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            global_step += 1
            (idx, bug_ids, bug_mask, context_ids, context_mask, gold_score) = batch
            _, _, _, train_loss = model(
                bug_ids=bug_ids.cuda(),
                bug_mask=bug_mask.cuda(),
...
if __name__ == "__main__":
    ...
    collator_function = src.data.RetrieverCollator(
        tokenizer, 
        context_max_length=opt.context_max_length,
        question_max_length=opt.question_max_length,
    )
    ...
    config = src.model.RetrieverConfig(
        ...
        bug_max_length=opt.bug_max_length,
        context_max_length=opt.context_max_length, 
