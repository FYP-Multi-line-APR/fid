...
# FIXME: format is changed [BUG] [ERROR] [CONTEXT]
class Dataset(torch.utils.data.Dataset):
    ...
    # FIXME: </s>  removed from target 
    def get_target(self, example):
        if 'fix' in example:
            target = example['fix']
            return target 
        elif 'fixes' in example:
            return random.choice(example['fixes'])
        else:
            return None
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        def append_bug(example):
            ...
            # ERROR part removed here for now
            # return [example['bug'] + " " + t for t in example['contexts']]
            return [t for t in example['contexts']]
